...
function arrayLikeKeys(value, inherited) {
  ...
  for (var key in value) {
    if ((inherited || hasOwnProperty$2.call(value, key)) && !(skipIndexes && (key == 'length' || isIndex$1(key, length)))) {
      result.push(key);
    }
  }
  return result;
...
function isSymbol$2(value) {
  return (typeof value === 'undefined' ? 'undefined' : _typeof(value)) == 'symbol' || isObjectLike$2(value) && objectToString$2.call(value) == symbolTag$2;
}
...
var omit = baseRest(function (object, props) {
  ...
  props = arrayMap(baseFlatten(props, 1), toKey$2);
...
var loggers = new index$1();
var configs = new index$1();
var wrappers = [];
var plugins = [];
...
function sendMessage(id, level, fallbackToLog) {
  var options = index$3({}, index$2(configs, 'namespaces.' + id, {}), { disable: index$2(configs, 'disable', []) }, index$5(configs, ['namespaces', 'disable']), { disable: configs.useGlobal ? index$2(global, 'logtown.namespaces.' + id + '.disable', []) : [] });
  options.disable = options.disable.map(function (d) {
    return d.toUpperCase();
  });
  var tagsToDisable = index$2(configs, 'tags.disable', []);
  var namespaceTags = index$2(configs, 'namespaces.' + id + '.tags', []);
  ...
  wrappers.concat(options.wrappers).forEach(function (wrapper) {
    if (typeof wrapper[levelMethod] === 'function') {
      return wrapper[levelMethod].apply(wrapper, toConsumableArray(logArgsWithoutLevel));
    } else if (typeof wrapper.log === 'function' && !!fallbackToLog) {
      return wrapper.log.apply(wrapper, toConsumableArray(logArgs));
    }
  });
}
...
function calcStats() {
  return {
    maxIdLength: Math.max.apply(Math, toConsumableArray(Object.keys(loggers).map(function (l) {
      return l.length;
    })))
  };
}
...
function applyPlugins(id, level, stats) {
  ...
  return plugins.reduce(function (acc, pluginFn) {
    ...
    return pluginFn.apply(undefined, toConsumableArray(acc));
...
function getLogger(id) {
  ...
  index$4(configs, 'namespaces.' + id, index$3(index$2(configs, 'namespaces.' + id, {}), config));
...
function configure() {
  ...
  var config = {
    ...
    tags: { disable: normalizeArray(index$2(tags, 'disable', [])) }
  };
  index$3(configs, config);
}
